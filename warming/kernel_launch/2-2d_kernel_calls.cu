...
int main() {
  ...
  float *host_buffer = (float*) std::malloc(size);  //FIXME
  //@@ Copy the buffer content from device to host
  //@@ use cudaMemcpy2D
  cudaMemcpy2D(host_buffer, cols*sizeof(float), d_buffer, pitch,
